...
#        self.SetBackgroundColour(wx.NamedColor("WHITE"))
        ...
        try:
            if os.path.exists(self.parent.movie.datadir+'coords.txt'): fname=self.parent.movie.datadir+'coords.txt'
            if os.path.exists(self.parent.movie.datadir+'stackcoord.txt'): fname=self.parent.movie.datadir+'stackcoord.txt'
            if os.path.exists(self.parent.movie.datadir+'coord3D.txt'): fname=self.parent.movie.datadir+'coord3D.txt'
            print fname
            header=rt.txtheader(fname)
            if len(header)>0: 
                xInd=header['x']
                yInd=header['y']
            ...
            print cs
            self.axes.scatter(xs, ys, zs,s=ss, color=cm.jet(cs))
            trajfiles=glob(self.parent.movie.datadir+'trajectory??????.txt')
            for f in trajfiles:
                d=np.loadtxt(f)
                hDic=rt.txtheader(f)
                try: self.axes.plot(d[:,hDic['x']],d[:,hDic['y']], zs=d[:,hDic['z']])
                except KeyError:
                    print hDic
                    print f
...
class MyImage(wx.StaticBitmap):
    ...
    def OnLeftDown(self,event):
        ...
        if ctrlstate: #record click position for director analysis!
            ...
            para=self.pparent.parameters
            blobs,contours=rt.extract_blobs(self.pparent.images['Threshold'], -1, para['blobsize'], para['sphericity'], diskfit=True,returnCont=True, outpSpac=1) 
            if len(blobs)>0:
                idx=np.argmin((x-blobs[:,3])**2+(y-blobs[:,4])**2)
                ctx,cty=blobs[idx][3],blobs[idx][4]
                #in theory, we could also take the closest blob centre, but we'll make this work first.
                with open(self.pparent.movie.datadir+'coordlog.txt', 'a') as f:
                    f.write("%d %.2f %.2f %.2f %.2f\n"%(self.pparent.framenum,ctx, cty, x, y))
                self.pparent.sb.SetStatusText("blob: x %d y %d, click: x %d y %d\n"%(ctx, cty, x, y), 0)
            else: 
                self.pparent.sb.SetStatusText("No blobs found!", 0)
...
class MyFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.frameMinMaxContr=wx.TextCtrl(paraPanel,215,'',size=(60,-1),style=wx.TE_PROCESS_ENTER)
        frameSpacLabel=wx.StaticText(paraPanel,-1,'Spacing, anal.')
        self.frameSpacContr=wx.TextCtrl(paraPanel,210,'',size=(40,-1),style=wx.TE_PROCESS_ENTER)
        navSpacLabel=wx.StaticText(paraPanel,-1,'Spacing, nav.')
        self.navSpacContr=wx.TextCtrl(paraPanel,216,'1',size=(40,-1),style=wx.TE_PROCESS_ENTER)
        ...
        hboxFrames.Add(navSpacLabel, 0, wx.ALIGN_RIGHT|wx.ALL,5)
        hboxFrames.Add(self.navSpacContr, 0, wx.ALIGN_RIGHT|wx.ALL,5)
    ...
    def OnKeyDown(self,event):
        ...
        if ctrlstate:
            if key==wx.WXK_LEFT:
                event.SetId(-1) #TODO This is dangerous. Should be a different spacing from analysis value
                try: spacing=int(self.navSpacContr.GetValue())
                except: 
                    spacing=1
                    self.navSpacContr.SetValue('1')
                if 0<=self.framenum-spacing<self.parameters['frames']:
                    self.framenum-=spacing
    ...
    def OpenMovie(self,event=None):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if not os.path.exists(self.movie.datadir):
                os.mkdir(self.movie.datadir)
            with open(self.movie.datadir+'coordlog.txt', 'w') as f:
                f.write('#frame blob_x blob_y defect_x defect_y\n')
    ...
    def OpenStackMovie(self,event=None):
        dlg = wx.DirDialog(self, "Select directory containing stack images", self.cdir, style=wx.DD_DEFAULT_STYLE | wx.DD_DIR_MUST_EXIST) #
        if dlg.ShowModal() == wx.ID_OK:
            self.moviefile=dlg.GetPath() #actually, this is a directory. However, as the the parent class uses files, we don't rename the variable
            if self.moviefile[-1]!=os.sep: self.moviefile+=os.sep #make sure the string ends with a path separator
            self.cdir=os.sep.join(self.moviefile.split(os.sep)[:-2])+os.sep #this is the parent directory containing image and data dirs
            print self.cdir
            if os.name=='posix':
                with open(os.path.abspath(__file__), 'r') as f:
                    text=f.read()
            ...
            if self.imType=='Background':
                if type(self.movie.bg).__name__!='ndarray':
                    if os.path.exists(self.movie.datadir+'bg.png'):
                        ...
                        if num>50: num=50
                        print num
                        if BGrng[1]<0 or BGrng[1]==255: bg=self.movie.getBGold(cutoff=BGrng[1], num=num, spac=int(self.parameters['frames']/(num+1)), prerun=30, save=True,channel=self.parameters['channel'])
...
#                                print contours[b]
    ...
    def ReadParas(self,event):
        ...
        #frame number
        if evID in [202,203,204,205]:
            if evID==202: num=self.frameSldr.GetValue() #slider
            if evID==203: num=self.framenum+1 #fwd Button
            if evID==204: num=self.framenum-1 #back Button
            if evID==205: num==int(self.frameContr.GetValue()) #text field
            if 0<=num<self.parameters['frames']:
                self.framenum=num
        if evID==206:
        ...
        try: self.infoWin.Update()
            ...
            self.infoWin.Show()
            self.infoWin.Update()
        if evID in range(200,800): self.infoWin.Raise()
    ...
    def GetClusters(self,event):
        ...
        self.parameters['channel']=int(self.channelCB.GetValue())
        self.movie.getClusters(thresh=self.parameters['threshold'],gkern=self.parameters['blur'],clsize=self.parameters['blobsize'],channel=self.parameters['channel'],rng=self.parameters['framelim'],spacing=self.parameters['spacing'], maskfile=mask, circ=self.parameters['circle'],imgspacing=self.parameters['imgspacing'])
        self.sb.SetStatusText(self.moviefile, 1)
        self.getCluB.Enable()
    ...
    def GetTrajectories(self,event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            with open(fname,'r') as f: 
                ...
                if rt.COORDHEADER2D==fileheader: flag=True
        else:
            wx.MessageBox('Please provide/generate a coordinate file (via Get Coordinates).', 'Info', wx.OK | wx.ICON_INFORMATION)
        if flag:
            self.movie.CoordtoTraj(coordFile=fname,lenlim=self.parameters['lenlim'], delete=True, maxdist=self.parameters['maxdist'], lossmargin=self.parameters['lossmargin'], spacing=self.parameters['spacing'])
        else:
            wx.MessageBox('Please provide/generate a coordinate file (via Get Coordinates).', 'Info', wx.OK | wx.ICON_INFORMATION)
        self.sb.SetStatusText(self.moviefile, 1)
    ...
    def OnHelp(self,event): #from Robin Dunn's wrapper
        cachedir = tempfile.gettempdir()
        wx.FileSystem.AddHandler(wx.ZipFSHandler())
        self.helpctrl = wx.html.HtmlHelpController()
        self.helpctrl.SetTempDir(cachedir)
        self.helpctrl.AddBook(helpzip, 1)
        self.helpctrl.SetTitleFormat("Movie analysis help")
